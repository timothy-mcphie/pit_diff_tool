...
def pit_diff(old_commit, new_commit, repo_path, old_rep, new_rep):
    check_report(old_rep)
    check_report(new_rep)
    if run_cmd(["git", "-C", repo_path, "status"]) != 0:
    ...
    return get_differences(old_rep, new_rep, old_commit+" -> "+new_commit)
